...
public class Login extends JFrame {
	 ...
	 */
	public Login() {
		setIconImage(Toolkit.getDefaultToolkit().getImage(Login.class.getResource("/images/ficon1.png")));
		...
		passtxt.setBounds(108, 162, 65, 16);
		...
		login.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				String ins="select password,user from airlines where ?=user";
				...
				String ouser=rs.getString("user");
				if(ouser.equals(str))
				{
				if(pass.equals(opass))
				{
					selection s=new selection(str);
					s.show();
					dispose();
				}
				else
		...
		signinbtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				airlinessignin as=new airlinessignin();
				as.show();
				dispose();
			}
		});
		...
		lblNewLabel.setIcon(new ImageIcon(Login.class.getResource("/images/Background12.jpg")));
		lblNewLabel.setBounds(0, 0, 786, 609);
